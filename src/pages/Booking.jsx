...
const getFlightsEndpoint = import.meta.env.VITE_GET_FLIGHTS_ENDPOINT;
...
export default function Booking({ onSearch }) {
	...
	const handleSubmit = async (e) => {
		...
		try {
			setFlights([]);
			const response = await fetch(getFlightsEndpoint, {
				method: 'POST',
				headers: {
					...
					<ul>
						{flights.map((flight, index) => (
							<li key={index} className="flight-card">
								...
								<br />
								{flight.DepartureCity} ({new Date(flight.DepartureTime).toLocaleString()}) →
								{flight.ArrivalCity} ({new Date(flight.ArrivalTime).toLocaleString()})
								<br />
